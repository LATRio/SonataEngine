...
void Renderer2D::Init()
{
    ...
    s_Data.QuadVertexBuffer = VertexBuffer::Create(Renderer2DData::MaxVertices * static_cast<int64_t>(sizeof(QuadVertex)));
...
void Renderer2D::EndScene()
{
    ...
    const int64_t dataSize{reinterpret_cast<uint8_t*>(s_Data.QuadVertexBufferPtr) -
...
#ifdef NON_BATCH
    s_Data.TextureShader->SetFloat4("u_TintColor", p_Color);
    s_Data.TextureShader->SetFloat("u_TilingFactor", 1.0f);

    const glm::mat4 transform =
        glm::translate(glm::mat4(1.0f), p_Position) * glm::scale(glm::mat4(1.0f), glm::vec3(p_Size.x,
        p_Size.y, 1.0f));
...
void Renderer2D::DrawQuad(const glm::vec2& p_Position, const glm::vec2& p_Size, const Ref<Texture2D>& p_Texture, const float p_TilingFactor)
{
    DrawQuad({p_Position.x, p_Position.y, 0.0f}, p_Size, p_Texture);
}

void Renderer2D::DrawQuad(const glm::vec3& p_Position, const glm::vec2& p_Size, const Ref<Texture2D>& p_Texture, const float p_TilingFactor)
{
    SN_PROFILE_FUNCTION();
    ...
    const float p_TilingFactor)
{
    ...
    s_Data.TextureShader->SetFloat4("u_TintColor", s_TintColor);
    s_Data.TextureShader->SetFloat("u_TilingFactor", p_TilingFactor);
